...
describe('AnalyticsService - Date Filtering', () => {
  ...
  describe('filterTransactionsByTimeRange', () => {
    test('should filter transactions for "week" correctly', () => {
      const filtered = analyticsService.filterTransactionsByTimeRange(sampleTransactions, 'week');
      
      // Should only include transactions from Aug 1-3, 2025
      expect(filtered).toHaveLength(2);
      expect(filtered.map(t => t.description)).toEqual(['Grocery Shopping', 'Salary']);
    });

    test('should filter transactions for "month" correctly', () => {
      const filtered = analyticsService.filterTransactionsByTimeRange(sampleTransactions, 'month');
    ...
    test('should filter transactions for "year" correctly', () => {
      const filtered = analyticsService.filterTransactionsByTimeRange(sampleTransactions, 'year');
    ...
    test('should return all transactions for "all"', () => {
      const filtered = analyticsService.filterTransactionsByTimeRange(sampleTransactions, 'all');
    ...
    test('should handle empty transactions array', () => {
      const filtered = analyticsService.filterTransactionsByTimeRange([], 'week');
      expect(filtered).toHaveLength(0);
    });

    test('should handle invalid timeRange', () => {
      const filtered = analyticsService.filterTransactionsByTimeRange(sampleTransactions, 'invalid');
      expect(filtered).toEqual(sampleTransactions);
    });
  });
  ...
  describe('calculateAllAnalytics', () => {
    test('should calculate analytics for week correctly', () => {
      const analytics = analyticsService.calculateAllAnalytics(sampleTransactions, 'week');
      
      expect(analytics.quickAnalytics.transactionCount).toBe(2);
      expect(analytics.quickAnalytics.income).toBe(5000.00);
      expect(analytics.quickAnalytics.spending).toBe(150.00);
      expect(analytics.quickAnalytics.netSavings).toBe(4850.00);
    });

    test('should calculate analytics for month correctly', () => {
      const analytics = analyticsService.calculateAllAnalytics(sampleTransactions, 'month');
    ...
    test('should calculate analytics for year correctly', () => {
      const analytics = analyticsService.calculateAllAnalytics(sampleTransactions, 'year');
      
      expect(analytics.quickAnalytics.transactionCount).toBe(5);
      expect(analytics.quickAnalytics.income).toBe(5000.00);
      expect(analytics.quickAnalytics.spending).toBe(305.00);
      expect(analytics.quickAnalytics.netSavings).toBe(4695.00);
    });
  });
  ...
  describe('Date handling', () => {
    test('should handle different date formats', () => {
      ...
      const filtered = analyticsService.filterTransactionsByTimeRange(transactionsWithDifferentFormats, 'week');
      expect(filtered).toHaveLength(2);
    });
    ...
    test('should handle invalid dates gracefully', () => {
      ...
      const filtered = analyticsService.filterTransactionsByTimeRange(transactionsWithInvalidDates, 'week');
