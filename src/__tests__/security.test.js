...
describe("Firebase Security Tests", () => {
  ...
  describe("Input Validation", () => {
    ...
    test("should reject very old transactions", () => {
      ...
      expect(errors).toContain(
        "Cannot create transactions older than 10 years"
      );
    });
  ...
  describe("Account Validation", () => {
    ...
    test("should reject accounts with excessive balances", () => {
      ...
      expect(errors).toContain(
        "Balance must be between -$1,000,000 and $1,000,000"
      );
    });
  ...
  describe("User Ownership Validation", () => {
    test("should validate user ownership correctly", () => {
      const isValid = SecurityMiddleware.validateUserOwnership(
        mockUserId,
        mockUserId
      );
    ...
    test("should check resource permissions", () => {
      ...
      const hasPermission = SecurityMiddleware.hasPermission(
        resource,
        mockUserId
      );
  ...
  describe("Rate Limiting", () => {
    test("should allow requests within rate limits", () => {
      ...
      for (let i = 0; i < 50; i++) {
        const canMakeRequest = SecurityMiddleware.checkRateLimit(
          mockUserId,
          "transactions"
        );
    ...
    test("should have different limits for different operations", () => {
      ...
      const canMakeAuthRequest = SecurityMiddleware.checkRateLimit(
        mockUserId,
        "auth"
      );
      ...
      const canMakeTransactionRequest = SecurityMiddleware.checkRateLimit(
        mockUserId,
        "transactions"
      );
  ...
  describe("Data Validation and Sanitization", () => {
    test("should validate and sanitize transaction data", async () => {
      ...
      expect(sanitizedData.amount).toBe(50.0);
