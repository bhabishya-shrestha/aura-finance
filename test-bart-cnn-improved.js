...
Please extract ALL financial transactions found in the text above using the exact format shown. Do not skip any transactions.`;
      ...
      if (response.ok) {
        ...
        if (data[0]?.summary_text) {
          ...
          const transactionCount = (data[0].summary_text.match(/Transaction \d+:/g) || []).length;
    ...
    try {
      ...
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          inputs: "Extract ALL financial transactions from this bank statement: EVEREST FOOD TRUCK 2 - $27.96 on 08/02/2025. WM SUPERCENTER #475 ROUND ROCK TX - $2.50 on 08/02/2025.",
          parameters: {
            max_length: 800,
            min_length: 100,
            do_sample: false,
