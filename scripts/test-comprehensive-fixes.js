...
async function testComprehensiveFixes() {
  ...
  for (const testCase of testCases) {
    ...
    try {
      // Test validation
      const errors = SecurityMiddleware.validateTransaction(testCase.transaction);
      ...
      if (errors.length === testCase.expectedErrors) {
        ...
        console.log(`  ❌ Validation result unexpected - expected ${testCase.expectedErrors} errors, got ${errors.length}`);
      }

      // Test sanitization (only for transactions that should pass validation)
      if (testCase.expectedErrors === 0) {
        const sanitized = SecurityMiddleware.sanitizeTransaction(testCase.transaction);
        console.log(`  Original date type: ${typeof testCase.transaction.date}`);
        console.log(`  Sanitized date type: ${typeof sanitized.date}`);
        console.log(`  Is sanitized date a Date object: ${sanitized.date instanceof Date}`);
        console.log(`  Original category: ${testCase.transaction.category || 'undefined'}`);
        ...
        if (sanitizedErrors.length === 0) {
          ...
          console.log(`  ❌ Sanitized validation failed: ${sanitizedErrors.join(', ')}`);
        }
      }
