...
try {
  ...
  const firebaseTransactions = firebaseTransactionsResult.success
    ? firebaseTransactionsResult.data || []
    : [];
  const firebaseAccounts = firebaseAccountsResult.success
    ? firebaseAccountsResult.data || []
    : [];

  console.log(
    `   Firebase has ${firebaseTransactions.length} transactions and ${firebaseAccounts.length} accounts`
  );
  ...
  const missingInFirebase = transactions.filter(
    t => !firebaseTransactionIds.has(t.id)
  );
  const missingInLocal = firebaseTransactions.filter(
    t => !localTransactionIds.has(t.id)
  );

  console.log(`\n📋 Sync Analysis:`);
  console.log(
    `   Transactions missing in Firebase: ${missingInFirebase.length}`
  );
    ...
    for (const transaction of missingInFirebase) {
      try {
        ...
        if (result.success) {
          ...
          failedCount++;
          console.log(
            `   ❌ Failed to upload: ${transaction.description} - ${result.error}`
          );
        }
      } catch (error) {
        failedCount++;
        console.log(
          `   ❌ Error uploading: ${transaction.description} - ${error.message}`
        );
      }
    }
  ...
  const missingAccountsInFirebase = accounts.filter(
    a => !firebaseAccountIds.has(a.id)
  );
  const missingAccountsInLocal = firebaseAccounts.filter(
    a => !localAccountIds.has(a.id)
  );

  console.log(`\n🏦 Account Sync Analysis:`);
  console.log(
    `   Accounts missing in Firebase: ${missingAccountsInFirebase.length}`
  );
    ...
    for (const account of missingAccountsInFirebase) {
      try {
        ...
        if (result.success) {
          ...
          failedCount++;
          console.log(
            `   ❌ Failed to upload: ${account.name} - ${result.error}`
          );
        }
      } catch (error) {
        failedCount++;
        console.log(
          `   ❌ Error uploading: ${account.name} - ${error.message}`
        );
      }
    }
  ...
  console.log(
    `   Final state: ${finalTransactions.length} transactions, ${finalAccounts.length} accounts`
  );

  // Check for any remaining sync issues
  const finalFirebaseTransactionsResult =
    await firebaseService.getTransactions();
  const finalFirebaseAccountsResult = await firebaseService.getAccounts();

  const finalFirebaseTransactions = finalFirebaseTransactionsResult.success
    ? finalFirebaseTransactionsResult.data || []
    : [];
  const finalFirebaseAccounts = finalFirebaseAccountsResult.success
    ? finalFirebaseAccountsResult.data || []
    : [];

  const transactionSyncDiff = Math.abs(
    finalTransactions.length - finalFirebaseTransactions.length
  );
  const accountSyncDiff = Math.abs(
    finalAccounts.length - finalFirebaseAccounts.length
  );
...
console.log(
  "💡 Try updating transaction categories again - it should work now."
);
