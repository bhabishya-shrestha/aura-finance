...
async function debugTransactionData() {
  ...
  try {
    ...
    console.log(`📊 Found ${transactions.length} transactions in local storage\n`);
    ...
    for (let i = 0; i < transactions.length; i++) {
      ...
      if (transaction.date) {
        if (transaction.date instanceof Date) {
          console.log(`  ✅ Valid Date object: ${transaction.date.toISOString()}`);
          validDates++;
          ...
          if (isNaN(parsedDate.getTime())) {
            ...
            console.log(`  ✅ Valid date string: ${transaction.date} -> ${parsedDate.toISOString()}`);
            validDates++;
          }
          ...
          if (isNaN(parsedDate.getTime())) {
            ...
            console.log(`  ✅ Valid timestamp: ${transaction.date} -> ${parsedDate.toISOString()}`);
            validDates++;
          }
        } else if (transaction.date && typeof transaction.date.toDate === 'function') {
          console.log(`  ✅ Firebase Timestamp: ${transaction.date.toDate().toISOString()}`);
          validDates++;
        } else {
    ...
    for (let i = 0; i < transactions.length; i++) {
      ...
      if (!transaction.date) {
        hasDateIssues = true;
      } else if (typeof transaction.date === 'string' && isNaN(new Date(transaction.date).getTime())) {
        hasDateIssues = true;
      } else if (typeof transaction.date === 'number' && isNaN(new Date(transaction.date).getTime())) {
        hasDateIssues = true;
      }
      ...
      if (hasDateIssues) {
        ...
        try {
          ...
          console.log(`  Sanitized date: ${sanitized.date instanceof Date ? sanitized.date.toISOString() : JSON.stringify(sanitized.date)}`);
          ...
          if (errors.length > 0) {
            console.log(`  ❌ Still has validation errors: ${errors.join(', ')}`);
          } else {
