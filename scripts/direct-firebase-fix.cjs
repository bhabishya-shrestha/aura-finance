...
async function initializeFirebase() {
  try {
    ...
    console.log('   3. Or download a service account key from Firebase Console');
    return false;
  }
}
  ...
  try {
    for (const transaction of missingTransactions) {
      ...
      if (!doc.exists) {
        ...
        console.log(`   ✅ Will create: ${transaction.description} (${transaction.id})`);
      } else {
        console.log(`   ℹ️  Already exists: ${transaction.description} (${transaction.id})`);
      }
    }
...
for (const transaction of transactions) {
  ...
  if (success) {
    ...
    console.log('💡 Your transaction category updates should now work without errors.');
