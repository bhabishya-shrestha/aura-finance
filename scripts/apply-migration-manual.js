...
function extractMigrationStatements() {
  ...
  try {
    ...
    console.log(
      "- Some statements may fail if objects already exist (this is normal)"
    );
    console.log(
      "- The migration will add denormalized columns to improve performance"
    );
    console.log(
      "- After migration, run the data pipeline to populate the new columns"
    );
  } catch (error) {
...
Examples:
  node apply-migration-manual.js extract
    ...
    default:
      ...
      console.log(
        'Run "node apply-migration-manual.js help" for usage information'
      );
      process.exit(1);
  }
}
